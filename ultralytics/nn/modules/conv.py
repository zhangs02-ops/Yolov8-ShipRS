...
class BSA(nn.Module):
    ...
    本模块利用船舶（高频突变）与海面背景（低频平滑）的频率差异， 通过低频分支估计背景强度，生成背景门控图，然后对高频前景特征 进行门控加权，以标准残差形式增强原始特征。
...
class SPDConv(nn.Module):
    ...
    替代传统 stride=2 下采样，避免小目标特征在下采样时丢失。 将空间维度(H,W)上的 2×2 区域重排到通道维度，实现无信息损失的下采样。
...
class LSK(nn.Module):
    ...
    并行使用局部小核(5×5)和大感受野空洞核(7×7,d=3)， 通过自适应加权让网络根据目标大小自动选择感受野。 对细长小目标（如船舶）特别有效。
    ...
    def __init__(self, c):
        super().__init__()
        # 局部精细分支
        self.local = nn.Sequential(nn.Conv2d(c, c, 5, padding=2, groups=c, bias=False), nn.BatchNorm2d(c), nn.SiLU())
        # 全局上下文分支（大核空洞卷积）
        self.global_ = nn.Sequential(
            nn.Conv2d(c, c, 7, padding=9, groups=c, dilation=3, bias=False), nn.BatchNorm2d(c), nn.SiLU()
        )
        # 自适应选择器
...
class BiFPNConcat(nn.Module):
    ...
    在标准 Concat 的基础上，为每个输入分支添加可学习的缩放权重， 通过快速归一化（Fast Normalized Fusion）实现自适应加权融合。

    标准 Concat 对所有输入一视同仁地拼接，BiFPNConcat 则让模型学习 不同来源特征的最优融合比例。对于遥感船舶检测，FPN/PAN 中不同尺度 的特征重要性差异显著（小目标主要依赖高分辨率特征），加权融合能
    自适应地强调更重要的特征来源。
...
class DySample(nn.Module):
    ...
    Learns dynamic sampling offsets for content-aware upsampling. Much lighter than SOAU (~16K params for 3 layers vs
    6.2M).
...
class CDGM(nn.Module):
    ...
    在FPN特征融合中，用高分辨率Backbone特征(detail)引导低分辨率上采样特征(coarse)的调制， 替代标准Concat实现更精细的跨尺度融合。
    ...
    参数量: c_detail × (c_detail//4 + 1) 个参数 使用方式: 在YAML中替换FPN路径的Concat: [[-1, N], 1, CDGM, []]
    """
    ...
    def forward(self, x):
        ...
        if coarse.shape[-2:] != detail.shape[-2:]:
            coarse = F.interpolate(coarse, size=detail.shape[-2:], mode="bilinear", align_corners=False)
...
class GSConv(nn.Module):
    ...
    标准 Conv(C, C_out, 3) → 参数量 = C × C_out × 9 GSConv(C, C_out, 3) → 参数量 ≈ C × C_out×0.5 + C_out×0.5×9 ≈ 50% 同时通过
    Channel Shuffle 保证多分支信息流通。
...
class ASG(nn.Module):
    ...
    在检测头前对每个尺度的特征图进行逐位置自适应门控。 让网络为每个空间位置评估"当前尺度的特征是否可靠"， 从而抑制噪声响应、增强可靠检测。
    ...
    参数量: ~c×9 + c×1 (DWConv + Pointwise)，约(10×c)个参数 使用方式: 在YAML中放在检测头前: [-1, 1, ASG, []]
    """
