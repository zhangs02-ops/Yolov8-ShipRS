...
class C2f_Lite(C2f):
    ...
    BottleneckLite 同时使用 RepVGG 重参数化和 PConv 部分卷积， 推理时融合为单卷积，在几乎不损失精度的情况下大幅降低计算量。

    使用方式: 在YAML中替换 C2f → C2f_Lite 即可。
    """

    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = False, g: int = 1, e: float = 0.5):
        super().__init__(c1, c2, n=n, shortcut=shortcut, g=g, e=e)
        self.m = nn.ModuleList(BottleneckLite(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
