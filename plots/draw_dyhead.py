...
def draw_box(ax, x, y, w, h, text, color="#E8F4FD", edge="#2196F3", fontsize=9, bold=False):
    ...
    ax.text(x, y, text, ha="center", va="center", fontsize=fontsize, color="#333333", weight=weight, wrap=True)
    return box
...
def draw_dashed_arrow(ax, x1, y1, x2, y2, color="#FF9800"):
    """画虚线箭头（残差连接）."""
    ax.annotate("", xy=(x2, y2), xytext=(x1, y1), arrowprops=dict(arrowstyle="->", color=color, lw=1.5, linestyle="--"))
...
# --- 左侧: Standard Detect ---
ax1.text(2.5, 9.5, "Standard Detect", ha="center", va="center", fontsize=14, weight="bold", color="#666666")
...
# 直接检测
ax1.text(2.5, 7.5, "Direct Detection", ha="center", va="center", fontsize=10, color="#999999", style="italic")
for i in range(4):
    x = 1.2 + i * 0.8
...
# --- 右侧: DyHeadDetect ---
ax1.text(8.5, 9.5, "DyHeadDetect (Ours)", ha="center", va="center", fontsize=14, weight="bold", color="#1565C0")
...
# DyBlock
ax1.text(8.5, 7.5, "DyBlock × 4", ha="center", va="center", fontsize=10, color="#1565C0", weight="bold")
for i in range(4):
    x = 7.2 + i * 0.8
    draw_box(ax1, x, 6.8, 0.7, 0.5, "DyBlock", color="#E3F2FD", edge="#2196F3", fontsize=7, bold=True)
    draw_arrow(ax1, x, 6.55, x, 5.9)
...
# --- 中间对比箭头 ---
ax1.annotate("", xy=(5.5, 7.0), xytext=(4.5, 7.0), arrowprops=dict(arrowstyle="->", color="#4CAF50", lw=2.5))
ax1.text(5.0, 7.25, "+ DyBlock", ha="center", va="bottom", fontsize=11, weight="bold", color="#4CAF50")
...
ax1.text(6.0, 2.0, "DyHeadDetect 改进点", ha="center", va="center", fontsize=12, weight="bold", color="#333333")
...
for i, text in enumerate(improvements):
    ax1.text(6.0, 1.55 - i * 0.35, text, ha="center", va="center", fontsize=9.5, color="#555555")

plt.tight_layout()
fig1.savefig(
    "/home/zhangs02/ultralytics-8.4.21/v5/v5/plots/dyhead_overview.png", dpi=200, bbox_inches="tight", facecolor="white"
)
plt.close()
...
# 三分支并行空洞卷积
ax2.text(
    5.5, 8.75, "Parallel Multi-Scale DWConv", ha="center", va="center", fontsize=10, weight="bold", color="#1565C0"
)
...
ax2.text(5.5, 5.0, "SE Channel Attention", ha="center", va="center", fontsize=11, weight="bold", color="#C2185B")
...
ax2.text(
    5.5,
    3.95,
    "Channel-wise reweighting: x = x ⊗ SE(x)",
    ha="center",
    va="center",
    fontsize=8.5,
    color="#AD1457",
...
draw_box(
    ...
    2.2,
    0.5,
    "x_out × scale  +  residual\n(scale: learnable, init=0.1)",
    color="#FFF8E1",
    edge="#FFA000",
    fontsize=9,
)
...
ax2.text(9.8, 10.2, "Params (C=256)", ha="center", va="center", fontsize=10, weight="bold", color="#333333")
stats = [
    "pw_reduce:  32.8K",
    "3×DWConv:    3.5K",
    "pw_expand:  32.8K",
...
for i, s in enumerate(stats):
    ...
    ax2.text(
        9.8, 9.7 - i * 0.45, s, ha="center", va="center", fontsize=9, color=color, weight=weight, family="monospace"
    )
...
ax2.text(
    5.5,
    0.6,
    r"$y = x + \sigma \cdot \left[ \text{SE}\left( \text{SiLU}\left(\text{BN}\left(\text{Conv}_{1\times1}\left(\sum_{d=1}^{3}\text{DWConv}_d(\text{Conv}_{1\times1}(x))\right)\right)\right)\right) \right]$",
    ha="center",
    va="center",
    fontsize=10,
    color="#555555",
...
plt.tight_layout()
fig2.savefig(
    "/home/zhangs02/ultralytics-8.4.21/v5/v5/plots/dyblock_detail.png", dpi=200, bbox_inches="tight", facecolor="white"
)
plt.close()
