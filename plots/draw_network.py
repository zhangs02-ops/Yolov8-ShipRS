...
def box(ax, x, y, w, h, text, color="#E8F4FD", edge="#2196F3", fontsize=8, bold=False):
    ...
    ax.text(x, y, text, ha="center", va="center", fontsize=fontsize, color="#333333", weight=weight)
    return b
...
def arrow(ax, x1, y1, x2, y2, color="#666666", style="-"):
    """画箭头."""
    ax.annotate(
        "", xy=(x2, y2), xytext=(x1, y1), arrowprops=dict(arrowstyle="->", color=color, lw=1.2, linestyle=style)
    )
...
ax.set_title(
    "YOLOv8-P2-ShipRS Architecture\n(SPDConv + C2f_EMA + CDGM + DyHeadDetect)", fontsize=18, weight="bold", pad=15
)
...
plt.tight_layout()
fig.savefig(
    "/home/zhangs02/ultralytics-8.4.21/v5/v5/plots/network_architecture.png",
    dpi=200,
    bbox_inches="tight",
    facecolor="white",
)
