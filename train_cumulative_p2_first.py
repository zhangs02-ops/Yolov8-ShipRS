...
def run(name, yaml_path, desc, epochs):
    print(f"\n{'=' * 60}\n实验: {name}\n描述: {desc}\nEpochs: {epochs}\nYAML: {yaml_path}\n{'=' * 60}\n")
    try:
        results = YOLO(str(yaml_path)).train(
            **TRAIN_ARGS, epochs=epochs, project="runs/ablation_cumulative_p2_first", name=name
        )
        m = {
...
def main():
    ...
    print("=" * 85)
    print(
        f"{'实验':<24} {'Epochs':>7} {'mAP50':>8} {'Δ步进':>8} {'Δ累计':>8} {'mAP50-95':>10} {'P':>7} {'R':>7} {'Params':>9}"
    )
    print("-" * 85)
    ...
    for name, _, desc, epochs in EXPERIMENTS:
        ...
        if m:
            ...
            print(
                f"{name:<24} {epochs:>7} {m['mAP50']:>8.4f} {ds:>+8.4f} {dt:>+8.4f} {m['mAP50-95']:>10.4f} {m['precision']:>7.4f} {m['recall']:>7.4f} {m['params']:>9.0f}"
            )
            prev = m["mAP50"]
        else:
