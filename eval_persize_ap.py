...
def main():
    ...
    # ─── Summary ────────────────────────────────────────────────────
    print(f"\n{'=' * 110}")
    print("Per-Size AP (COCO) @ 640×640 on seaship val")
    print(f"{'=' * 110}")
    print(f"{'Experiment':<12} {'AP50':>8} {'AP50-95':>10} {'AP50-95_S':>10} {'AP50-95_M':>10} {'AP50-95_L':>10}")
    print(f"{'-' * 110}")
    ...
    for name, _ in EXPERIMENTS:
        ...
        if r is None:
            ...
            continue
        print(
            f"{name:<12} {r['AP50_all']:>8.4f} {r['AP50-95_all']:>10.4f} "
            f"{r['AP50-95_small']:>10.4f} {r['AP50-95_medium']:>10.4f} {r['AP50-95_large']:>10.4f}"
        )

    # delta table
    print(f"\n{'=' * 110}")
    print("Delta vs 01_p2")
    print(f"{'=' * 110}")
    base = all_results.get("01_p2", {})
    if base:
        print(
            f"{'Experiment':<12} {'ΔAP50':>8} {'ΔAP50-95':>10} {'ΔAP50-95_S':>10} {'ΔAP50-95_M':>10} {'ΔAP50-95_L':>10}"
        )
        print(f"{'-' * 110}")
        for name, _ in EXPERIMENTS:
            r = all_results.get(name)
            if r is None or name == "01_p2":
                ...
                continue
            print(
                f"{name:<12} {r['AP50_all'] - base['AP50_all']:>+8.4f} {r['AP50-95_all'] - base['AP50-95_all']:>+10.4f} "
